    ...
    def test_pparam_update_majority(
        ...
        * check that action 3 is enacted 
        ...
        if is_in_bootstrap and not configuration.HAS_CC:
            pytest.skip(
                "The test doesn't work in bootstrap period without CC.")
        ...
        delegated_stake = governance_utils.get_delegated_stake(
            cluster_obj=cluster)
        cur_pparams = cluster.g_conway_governance.query.gov_state()[
            "currentPParams"]
        ...
        [r.start(url=_url)
         for r in (reqc.cip049, reqc.cip050, reqc.cip051, reqc.cip052)]
        ...
        def _check_state(state: dict):
            pparams = state.get("curPParams") or state.get(
                "currentPParams") or {}
        ...
        try:
            ...
            [r.success() for r in (reqc.cli017, reqc.cip031a_05,
                                   reqc.cip031e, reqc.cip054_01)]
            ...
            if _cur_epoch == fin_approve_epoch + 1:
                ...
                for fin_prop_rec in fin_prop_recs:
                    ...
                    if action_ix == 0 or action_ix == 1:
                        ...
                        enacted_action = (enact_prev_action_rec.txid == fin_prop_rec.action_txid) and (
                            enact_prev_action_rec.ix == action_ix)
            ...
            # Check enactment
            assert _cur_epoch == fin_approve_epoch + \
                2, f"Unexpected epoch {_cur_epoch}"
            ...
            if "MaxTxSizeUTxO" in err_str:
                print(
                    f"Fails at proposing {(num_pool_users)} parameter updates actions in a single transaction")
                return
