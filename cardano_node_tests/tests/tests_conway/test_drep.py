...
class TestDelegDReps:
    ...
    def test_dreps_delegation(
        ...
        * check that the stake addresses are registered
        ...
        pool_users_skey_file = [
            pool_user.stake.skey_file for pool_user in pool_users]
        ...
        address_utxos = [cluster.g_query.get_utxo(
        ...
        try:
            ...
            if (
                ...
                for pool_user in pool_users:
                    ...
                    deleg_state = clusterlib_utils.get_delegation_state(
                        cluster_obj=cluster)
                    ...
                    deleg_amount = cluster.g_query.get_address_balance(
            ...
            if "MaxTxSizeUTxO" in err_str:
                print(
                    f"Fails at registering {len(reg_certs + deleg_certs)} certificates in a single transaction")
                return
    ...
    def test_dreps_and_spo_delegation(
        ...
        * check that the stake addresses are registered and delegated
        ...
        reqc.cli030.success()
        pool_users_wp_skey_file = [
            pool_user_wp.stake.skey_file for pool_user_wp in pool_users_wp]
        tx_files = clusterlib.TxFiles(
            certificate_files=reg_certs +
            list(chain.from_iterable(deleg_certs)),
            signing_key_files=[payment_addr_wp.skey_file] +
            pool_users_wp_skey_file,
        )
        ...
        address_utxos = [cluster.g_query.get_utxo(
            pool_user.payment.address) for pool_user in pool_users_wp]
        flatenned_utxos = list(chain.from_iterable(address_utxos))
        try:
            tx_output = clusterlib_utils.build_and_submit_tx(
                cluster_obj=cluster,
            ...
            assert (
                clusterlib.filter_utxos(
                    utxos=out_utxos, address=payment_addr_wp.address)[0].amount
            ...
            if "MaxTxSizeUTxO" in err_str:
                print(
                    f"Fails at registering {len(reg_certs + list(chain.from_iterable(deleg_certs)))} certificates in a single transaction")
                return
