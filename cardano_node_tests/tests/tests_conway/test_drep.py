...
def pool_users(
    ...
    return get_pool_user(
        name_template=test_id, cluster_manager=cluster_manager, cluster_obj=cluster, caching_key=key, no_of_users=30
    )
...
def pool_users_wp(
    ...
    key = helpers.get_current_line_str()
    return get_pool_user(
        name_template=test_id, cluster_manager=cluster_manager, cluster_obj=cluster, caching_key=key, no_of_users=15
    )
...
class TestDelegDReps:
    ...
    def test_dreps_delegation(
        ...
        * check that the stake address is registered
        ...
        total_pool_users = len(pool_users)
    ...
    def test_dreps_and_spo_delegation(
        ...
        * check that the stake address is registered and delegated
        ...
        for i in range(len(pool_users_wp)):
            ...
            assert pool_ids[i] == stake_addr_info.delegation, "Stake address delegated to wrong pool"
            assert stake_addr_info.vote_delegation == governance_utils.get_drep_cred_name(
                drep_id=drep_id
            ), "Votes are NOT delegated to the correct DRep"
        out_utxos = cluster.g_query.get_utxo(tx_raw_output=tx_output)
