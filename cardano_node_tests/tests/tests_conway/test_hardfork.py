...
class TestHardfork:
    ...
    def test_hardfork_majority(
        ...
        * check that it's not possible to vote on enacted action
        ...
        pool_user_skey_files = [
            pool_user_lg.payment.skey_file for pool_user_lg in pool_users_lg]
        tx_files_action = clusterlib.TxFiles(
            proposal_files=[
                hardfork_action.action_file for hardfork_action in hardfork_actions],
        ...
        address_utxos = [cluster.g_query.get_utxo(
            pool_user.payment.address) for pool_user in pool_users_lg]
        ...
        try:
            ...
            out_utxos_action = cluster.g_query.get_utxo(
            ...
            action_txid = cluster.g_transaction.get_txid(
            ...
            print(f"{len(governance_data.cc_key_members)} CC members are voting")
            ...
            while True:
                gov_state = cluster.g_conway_governance.query.gov_state()
                proposal_action = governance_utils.lookup_proposal(
                    gov_state=gov_state, action_txid=action_txid, action_ix=2)
                if proposal_action:
                    continue
                else:
                    break
            ...
            governance_utils.check_action_view(
                cluster_obj=cluster, action_data=hardfork_actions[2])

        except clusterlib.CLIError as exc:
            err_str = str(exc)
            if "MaxTxSizeUTxO" in err_str:
                print(
                    f"Fails at submitting {actions_num} hardfork actions in a single transaction")
                return
        ...
        * check that it's not possible to vote on enacted action
        ...
        try:
            ...
            while True:
                gov_state = cluster.g_conway_governance.query.gov_state()
                proposal_action = governance_utils.lookup_proposal(
                    gov_state=gov_state, action_txid=action_txid, action_ix=1)
                if proposal_action:
                    continue
                else:
                    break
            ...
            # Check action view
            governance_utils.check_action_view(
                cluster_obj=cluster, action_data=hardfork_actions[1])
        ...
        * check return account balance
        ...
        try:
            ...
            while True:
                gov_state = cluster.g_conway_governance.query.gov_state()
                proposal_action = governance_utils.lookup_proposal(
                    gov_state=gov_state, action_txid=action_txid, action_ix=0)
                if proposal_action:
                    continue
                else:
                    break
