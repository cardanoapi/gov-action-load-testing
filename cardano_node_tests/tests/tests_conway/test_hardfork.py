...
class TestHardfork:
    ...
    @pytest.mark.long
    @pytest.mark.parametrize("num_pool_users", reversed(range(117, 119)))
    def test_hardfork(
        self,
        ...
        * check that it's not possible to vote on enacted action
        ...
        print(f"\nSubmitting {len(hardfork_actions)} hardfork action proposals in a single transaction")
        ...
        try: 
            ...
            # Check action view
            governance_utils.check_action_view(cluster_obj=cluster, action_data=hardfork_actions[total_participants-1])
            ...
            if "MaxTxSizeUTxO" in err_str:
                print(f"Fails at submitting {len(hardfork_actions)} hardfork actions in a single transaction")
                return
